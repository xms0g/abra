...
Kernel::Kernel(const std::string& name, const float* kernel, const bool enabled) : IPostEffect(name, enabled),
	mKernel(kernel) {
...
uint32_t Kernel::render(const uint32_t sceneTexture, const uint32_t VAO,
